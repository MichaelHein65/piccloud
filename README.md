...
- Memory-aware full-screen loading: the viewer requests a server-side 4096 px image variant instead
  of decoding ultra-large originals on the phone.
...
The full-screen viewer does not load `/image/...` originals directly. It uses `/thumb/4096/...`,
which is generated by the server from the original image and is large enough for inspection while
avoiding iOS memory termination on very large photos.
